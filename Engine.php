...
use colo\core\Loader;
use colo\core\Dispatcher;
 ...
 * @method \colo\template\View view() Gets current view
 *
 * Request-response
...
class Engine {
    ...
    public function init() {
        ...
        $this->loader->register('request', '\colo\net\Request');
        $this->loader->register('response', '\colo\net\Response');
        $this->loader->register('router', '\colo\net\Router');
        $this->loader->register('view', '\colo\template\View', array(), function($view) use ($self) {
            $view->path = $self->get('colo.views.path');
            $view->extension = $self->get('colo.views.extension');
        });
        ...
        $this->set('colo.base_url', null);
        $this->set('colo.case_sensitive', false);
        $this->set('colo.handle_errors', true);
        $this->set('colo.log_errors', false);
        $this->set('colo.views.path', './views');
        $this->set('colo.views.extension', '.php');

        // Startup configuration
        $this->before('start', function() use ($self) {
            // Enable error handling
            if ($self->get('colo.handle_errors')) {
                set_error_handler(array($self, 'handleError'));
                set_exception_handler(array($self, 'handleException'));
            }

            // Set case-sensitivity
            $self->router()->case_sensitive = $self->get('colo.case_sensitive');
        });
